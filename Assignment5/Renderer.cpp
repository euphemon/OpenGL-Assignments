...
void Renderer::nanogui_init(GLFWwindow* window)
{
    ...
    static auto bone1_x_widget = gui_1->addVariable("Rotate X1", m_bone_animation->rotation_degree_vector[1][2]);
    static auto bone1_y_widget = gui_1->addVariable("Rotate Y1", m_bone_animation->rotation_degree_vector[1][0]);
    static auto bone1_z_widget = gui_1->addVariable("Rotate Z1", m_bone_animation->rotation_degree_vector[1][1]);
    
    static auto bone2_x_widget = gui_1->addVariable("Rotate X2", m_bone_animation->rotation_degree_vector[2][2]);
    static auto bone2_y_widget = gui_1->addVariable("Rotate Y2", m_bone_animation->rotation_degree_vector[2][0]);
    static auto bone2_z_widget = gui_1->addVariable("Rotate Z2", m_bone_animation->rotation_degree_vector[2][1]);
    
    static auto bone3_x_widget = gui_1->addVariable("Rotate X3", m_bone_animation->rotation_degree_vector[3][2]);
    static auto bone3_y_widget = gui_1->addVariable("Rotate Y3", m_bone_animation->rotation_degree_vector[3][0]);
    static auto bone3_z_widget = gui_1->addVariable("Rotate Z3", m_bone_animation->rotation_degree_vector[3][1]);

    gui_1->addButton("Reset Bone Rotation", []() {
        m_bone_animation->reset();
    });

	gui_1->addGroup("Code by: Euphemia Wu");
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	// Draw root bone
    std::vector<glm::mat4> root_bone_obj_mat = { glm::mat4(1.0f), glm::mat4(1.0f), glm::mat4(1.0f), glm::mat4(1.0f) };
    for (int i = 0; i < m_bone_animation->bone_height; i++)
    {
        root_bone_obj_mat[i] = glm::scale(m_bone_animation->bone_matrix[i], m_bone_animation->scale_vector[i]);
        glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(root_bone_obj_mat[i]));
