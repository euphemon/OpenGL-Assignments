...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Catmull-Rom curve on/off", m_curve->box_clicked);

	gui_1->addGroup("Aircraft Animation");
	gui_1->addVariable("aircraft is moving", m_aircraft_animation->is_active);
	gui_1->addVariable("T1", m_aircraft_animation->t1);
	gui_1->addVariable("T2", m_aircraft_animation->t2);
	gui_1->addVariable("total moving time", m_aircraft_animation->total_moving_time);
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		if (obj_list[i].obj_name == "cube")
		{
			...
			for (unsigned int j = 0; j < m_curve->control_points_pos.size(); j++) {
				...
				cur_obj_model_mat = m_curve->rotate_with_quaternion(cur_obj_model_mat, j);
...
void Renderer::update_curve_rendering()
{

	if (m_curve->box_clicked == true && obj_list.size() == 4) {
		m_curve->calculate_curve();
		Object curve_object(m_curve->curve_points_pos);
		curve_object.m_render_type = RENDER_LINES;
		curve_object.obj_color = glm::vec4(1.0, 0.0, 0.0, 1.0);
	...
	else if (m_curve->box_clicked == false && obj_list.size() == 5) {
		obj_list.pop_back();
	}
}
