...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	nanogui::ref<nanogui::Window> nanoguiWindow_1 = gui_1->addWindow(Eigen::Vector2i(0, 0), "Control Panel");
    ...
    gui_1->addGroup("Model Rotation");
    gui_1->addVariable("A", m_animation->angleX)->setSpinnable(true);
    gui_1->addVariable("B", m_animation->angleY)->setSpinnable(true);
    
    gui_1->addButton("Rotate A on LCS X-Axis", []() {
        
        m_animation->applyRotationOnLocalAxis();
    });
    
    gui_1->addButton("Rotate B on WCS Y-Axis", []() {
        
        m_animation->applyRotationOnWorldAxis();
    });
