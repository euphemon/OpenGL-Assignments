...
nanogui::detail::FormWidget<bool>* bone_move = nullptr;
...
nanogui::detail::FormWidget<float>* end_pos_X = nullptr;
nanogui::detail::FormWidget<float>* end_pos_Y = nullptr;
nanogui::detail::FormWidget<float>* end_pos_Z = nullptr;
...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	nanogui::ref<nanogui::Window> nanoguiWindow_1 = gui_1->addWindow(Eigen::Vector2i(0, 0), "Nanogui control bar_1, Code by Euphemia Wu");
	...
	gui_1->addButton("Reset Bone Rotation", []() {
		m_bone_animation->rotation_reset();
		...
		bone_move->setValue(m_bone_animation->movement_on);

		});

	bone_move = gui_1->addVariable("Bone Move", m_bone_animation->movement_on);

	target_X = gui_1->addVariable("Target X:", m_bone_animation->target_position[0]);
	target_Y = gui_1->addVariable("Target Y:", m_bone_animation->target_position[1]);
	target_Z = gui_1->addVariable("Target Z:", m_bone_animation->target_position[2]);

	end_pos_X = gui_1->addVariable("Current end pos X:", m_bone_animation->end_effector[0]);
	end_pos_Y = gui_1->addVariable("Current end pos Y:", m_bone_animation->end_effector[1]);
	end_pos_Z = gui_1->addVariable("Current end pos Z:", m_bone_animation->end_effector[2]);
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	std::vector<glm::mat4> bone_object = { glm::mat4(1.0f), glm::mat4(1.0f), glm::mat4(1.0f), glm::mat4(1.0f) };
	for (int i = 0; i < m_bone_animation->bone_height; i++)
	{
		bone_object[i] = glm::scale(m_bone_animation->bone_matrix[i], m_bone_animation->scale_vector[i]);
		glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone_object[i]));
		bone_obj->obj_color = m_bone_animation->colors[i];
		draw_object(shader, *bone_obj);
	}

	glm::mat4 target_object = glm::mat4(1.0f);
	target_object = glm::translate(target_object, m_bone_animation->target_position);
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(target_object));
...
void Renderer::update_value()
{
	...
	end_pos_X->setValue(m_bone_animation->end_effector[0]);
	end_pos_Y->setValue(m_bone_animation->end_effector[1]);
	end_pos_Z->setValue(m_bone_animation->end_effector[2]);
}
